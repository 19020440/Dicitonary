...
public class SampleController  {
	...
	public void initialize() {
		...
		wordSound.setOnAction(e -> {
			try {
				TextToSpeech tts = new TextToSpeech();
		        String word = wordName.getText();
		        tts.speak(word,2.0f,false, true);
			} catch (Exception ex) {
				System.out.println("soundButton. onAction. "+ wordName.getText());
			}
		});
...
//	}
	...
	public void getRecommendList() {
		...
		rs.insertFromMySQL(lists); // getWord by database
//		List<String> word_target = new ArrayList<String>();
//		List<String> word_explain = new ArrayList<String>();
//		word_target = rs.List("database.txt");
//		word_explain = rs.List("meaning.txt");
////		
//		for(int i=0;i<word_explain.size();++i) {
//			Word rs1 = new Word();
////			rs1.setWord_target(word_target.get(i));
////			rs1.setWord_explain(word_explain.get(i));
//			lists.words.add(rs1);
//		} // getWord by text Field
